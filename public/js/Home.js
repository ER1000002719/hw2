...
function sendComment(event){
    ...
    em.textContent = '@' + document.querySelector('[href="/Profile"]').textContent + ':';
    p.appendChild(em)
    p.insertAdjacentText('beforeend',CommentText.querySelector('textarea').value);

    CommentText.parentNode.appendChild(p);
    const Username = document.querySelector('[href="/Profile"]').textContent;
...
function fetchPosts() {
    fetch("/getPosts").then(onResponse).then(onJsonPosts);
}
